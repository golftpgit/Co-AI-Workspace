...
// Deliberately NOT using surrealdb.swift (alpha, no releases, remote-only).
// Surface needed by Co-AI Workspace: signin / use / query (+ live later).
//
// NOTE (design lesson from this spike): Swift 6 strict concurrency rejects
// `Any` across actor boundaries, so the wire type must be a Sendable enum.
// That turns out to be the right shape anyway — typed access, no casts.
// ─────────────────────────────────────────────────────────────
...
public actor SurrealClient {
    ...
    private func rpc(_ method: String, _ params: [Any], timeout: Double = 30) async throws -> SurrealValue {
        ...
        return try await withThrowingTaskGroup(of: SurrealValue.self) { group in
            group.addTask { [self] in
                try await withCheckedThrowingContinuation { (c: CheckedContinuation<SurrealValue, Error>) in
                    Task { await self.store(id: id, cont: c) }
                    Task {
            ...
            group.addTask {
                ...
                throw SurrealError.timeout(method: method)
            }
    ...
    private func store(id: String, cont: CheckedContinuation<SurrealValue, Error>) { pending[id] = cont }
    private func fail(id: String, error: Error) { pending.removeValue(forKey: id)?.resume(throwing: error) }
