...
### 17.1 GX10 — ค่าที่ตั้งจริง และสิ่งที่ระบบต้องอ่านจากมัน ไม่ใช่เดา

เครื่อง **ASUS Ascent GX10 ที่ `192.168.1.205`** เสิร์ฟด้วย vLLM ที่พอร์ต 8000 · โมเดล `Qwen3.8-27B-Fable-Distill-NVFP4` · `--max-model-len 32768` · `--kv-cache-dtype fp8` · `--enable-prefix-caching` · `--enable-chunked-prefill` · `--max-num-seqs 256`

**ธงสามอันในบรรทัดนั้นเปลี่ยนการออกแบบฝั่งแอป ไม่ใช่แค่ประสิทธิภาพของเซิร์ฟเวอร์**:

| ธง | ความหมายต่อฝั่งแอป |
|---|---|
| `--max-num-seqs 256` | เสิร์ฟงานขนานได้จริงหลายสิบสาย ⇒ **องค์กรหลายทีมใน [§22](#22-ai-organization--จากทีมเดียวเป็นองค์กร-m17-command) มีที่รันจริง** ไม่ใช่ทฤษฎี — คอขวดกลับไปอยู่ที่งบและ span of control ไม่ใช่ที่ฮาร์ดแวร์ |
| `--enable-prefix-caching` | prompt ที่ขึ้นต้นเหมือนกันถูกใช้ซ้ำ ⇒ **การวาง system prompt และรายการทูลให้คงที่ตอนต้นแล้วให้ส่วนที่เปลี่ยนอยู่ท้าย มีผลจริงกับความเร็วและค่าใช้จ่าย** ไม่ใช่ความสวยงามของโค้ด |
| `--max-model-len 32768` | เพดานนี้**รวมขาออก** — `ContextManager` ต้องอ่านค่าจาก endpoint ([§5.6.1](#561-กลยุทธ์บริบทเต็มรูปแบบ--compaction-เป็นข้อเดียวในสี่ข้อ)) ไม่ใช่ถือ 16,384 ที่เขียนตายไว้จากการวัดบนเครื่อง 16 GB |

**สิ่งที่ยังต้องพิสูจน์บนเครื่องจริง ไม่ใช่อนุมานจากสเปก** — บันทึกลง [`docs/VERIFICATION_LOG.md`](docs/VERIFICATION_LOG.md): tool-calling และ structured output ของโมเดลนี้ผ่าน `VLLMExecutor` เดิม · ความเร็วจริงเมื่อมีหลายสายพร้อมกัน · พฤติกรรมเมื่อ LAN หลุดกลางสตรีม (P9.4 ครอบ `URLError` ไว้แล้ว แต่ยังไม่เคยเจอกับ endpoint นี้)
...
API ที่ใช้ได้จริงบน macOS 26 คือ `.glassEffect(_:in:isEnabled:)` และ `GlassEffectContainer` (มีตั้งแต่ iOS/macOS 26) — **ตรวจกับ SDK บนเครื่องก่อนออกแบบทับ ไม่ใช่เชื่อจากบทความ**
