...
public struct Bibliography: Sendable {
    ...
    private static func locator(_ work: Provenance) -> [String] {
        ...
        if let page = work.page { parts.append(localised("p. \(page).", "Page number in a citation. Placeholder: the page.")) }
        ...
        if let passage = work.passage {
            parts.append(localised("lines \(passage.start)–\(passage.end).", "Line range in a citation. Placeholders: the first and last line."))
        }
        switch work.origin {
        case .web(let url):
            ...
            parts.append(localised("\(url.absoluteString) (accessed \(formatter.string(from: work.accessedAt))).", "A web citation. Placeholders: the URL and the access date."))
        case .upload(let filename):
            parts.append(localised("[file: \(filename)].", "A file citation. Placeholder: the filename."))
        case .database(let name):
            parts.append(localised("[database: \(name)].", "A database citation. Placeholder: the database name."))
        case .userAuthored:
            parts.append(localised("[generated by this system].", "A citation of the system's own output."))
        case .fieldwork(let participantCode):
            ...
            parts.append(participantCode.map { localised("[fieldwork: \($0)].", "A fieldwork citation. Placeholder: the study name.") }
                         ?? localised("[fieldwork].", "A fieldwork citation with no study name."))
        }
...
public struct CitationAudit: Sendable, Equatable {
    ...
    public static func audit(_ citations: [CitedText]) -> CitationAudit {
        ...
        for citation in citations {
            ...
            var fields: [String] = []
            if work.authors.isEmpty, !work.origin.isSelfAuthored { fields.append(localised("author", "A citation field that is missing.")) }
            if work.year == nil, !work.origin.isSelfAuthored { fields.append(localised("year", "A citation field that is missing.")) }
