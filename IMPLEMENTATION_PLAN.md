...
| **P2.8** Embedding profile + reindex *(ใหม่ 2026-08-11)* | โมเดล embedding ผูกกับ **index** ไม่ใช่ tier — เก็บ `modelID/revision/dimensions/pooling/normalised/tokenizerVersion/chunkerVersion` เป็น profile เดียว + job re-embed ตอนเปลี่ยนโมเดล | index ปฏิเสธ vector ที่มาจากโมเดลอื่น (ทั้งตอนเขียนและตอนค้น); เปลี่ยนโมเดลแล้ว re-embed จากข้อความที่เก็บไว้ได้โดยไม่ต้อง OCR ใหม่ และวัด recall เทียบก่อนสลับ | 🔶 **ครึ่งแรกเสร็จ** — `EmbeddingProfile` + การปฏิเสธ vector ข้าม profile (เขียน/ค้น/lexical-only) ครบพร้อมเทส 7 ตัว · **ยังไม่ทำ**: reindex job + golden-query gate |
