...
| **P4.8** Operating modes | Autonomy Slider / Plan-only / Run-until-done — อิสระต่อกัน มองเห็นชัดในหัว conversation | ทั้ง 3 สวิตช์ทำงานถูกต้องและไม่กวนกัน | 🔶 **โค้ดครบและมีเทส** — `OperatingModes` แยกสามสวิตช์ออกจากกันจริง (v1 รวม "ไม่รัน" กับ "ไม่ถาม" เข้าด้วยกันจนปิดอันหนึ่งแล้วอีกอันเปลี่ยนตาม), `planOnly` บังคับใน `ToolGateway` ไม่ใช่ที่ตัว agent, `runUntilDone` เป็นสวิตช์ชัดเจนไม่ใช่การเดา · ทั้งสามอยู่บนหัวหน้าแชทแล้ว · 🔴 **ค้าง — สวิตช์ที่สามไม่ทำอะไรเลย**: `runUntilDone` ถูกประกาศใน `OperatingModes` ส่งเข้า `ToolGateway` และมี toggle บนหน้าแชท แต่ **ไม่มีโค้ดบรรทัดไหนใน `Sources` อ่านค่ามัน และไม่มีเทสตัวไหนแตะมัน** — กดแล้วเปลี่ยนแค่ค่าที่เก็บไว้ ต่างจาก `autonomy` (บังคับใน `HookChain`) และ `planOnly` (บังคับทั้ง `HookChain` และ `ToolGateway`) · Done-when ข้อนี้คือ "ทั้ง 3 สวิตช์ทำงานถูกต้อง" จึงยังปิดไม่ได้ · **ต้องตัดสินใจก่อนทำ**: §5.5 กำหนดแค่ว่าสวิตช์ต้องเป็น explicit ไม่ auto-detect แต่ไม่ได้บอกว่า "จบงาน" วัดจากอะไร ซึ่งเป็นจุดที่ v1 พลาด |
