...
|---|---|---|---|
| **U1** | **หน้าจอ P2.7 ยังไม่เคยถูกขับจริง** | โค้ดครบ, build + เปิดแอปได้, ไม่ crash — แต่**ยังไม่มีใครกดใช้จริงสักครั้ง** ผู้ช่วยที่เขียนโค้ดนี้ capture หน้าจอไม่ได้ (ไม่มีสิทธิ์ Screen Recording/Accessibility) | เปิดแอป → แท็บคลังความรู้ → เพิ่มเอกสาร/ค้น/แก้ entity/ลบ/export/import ทีละอย่าง · **P1.10 เจอบั๊ก 8 ข้อด้วยวิธีนี้ที่เทส 139 ตัวมองไม่เห็น** |
| **U2** | **KB persistence ยังไม่ได้ทดสอบ end-to-end ผ่าน UI** | ชั้น `KnowledgeStore` มี 7 เทสกับ SurrealDB จริง แต่เส้นทาง UI → DB → ปิดแอป → เปิดใหม่ → เห็นของเดิม ยังไม่เคยรัน | ingest เอกสาร → ปิดแอป → เปิดใหม่ → ต้องเห็นเอกสารเดิมและค้นเจอ |
