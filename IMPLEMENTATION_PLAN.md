...
|---|---|---|---|
| **U1** | **ยังไม่มีใครกดหน้าจอจริงสักครั้ง** (คลังความรู้ + Conflict Card) | ตรรกะเบื้องหลังผ่านหมดแล้ว (ดู U2) แต่ **การเรนเดอร์และการโต้ตอบยังไม่เคยถูกตรวจ** · ผู้ช่วยขับ UI อัตโนมัติไม่ได้: macOS ปฏิเสธทั้ง Screen Recording และ Accessibility ให้โปรเซสที่รันคำสั่ง (`screencapture` → "could not create image from display", System Events เห็นแอปแต่รายงาน 0 หน้าต่าง) — **คำอนุญาตด้วยวาจาไม่เปลี่ยนสิทธิ์ระดับ OS** | ทางเลือก: (ก) คนกดเองสัก 10 นาที หรือ (ข) เปิดสิทธิ์ให้แอปเทอร์มินัลที่รัน Claude ใน System Settings → Privacy & Security → **Accessibility** และ **Screen Recording** แล้วรีสตาร์ทแอปนั้น ผู้ช่วยจะขับ UI เองได้ · **P1.10 เจอบั๊ก 8 ข้อด้วยวิธีนี้ที่เทส 139 ตัวมองไม่เห็น** ครึ่งหนึ่งเป็นเรื่องการเรนเดอร์ล้วนๆ |
| **U2** | KB persistence end-to-end | ✅ **ปิดแล้วในระดับตรรกะ** — `EmbeddingCheck` มี "เส้นทางที่หน้าจอใช้จริง" 7 ข้อ รันทุกครั้งที่ `check.sh`: เพิ่มเอกสาร → ค้นเจอ → แก้ entity แล้วผลเปลี่ยน → hybrid ใช้ฝั่ง vector จริง → ลบทั้งฉบับ → เขียนลง DB แล้วโหลดใหม่ยังอยู่ (พร้อม vector 1024 มิติ) → แก้ entity รอดข้ามการเปิดใหม่ → ตัดสินข้อขัดแย้งแล้วไม่ถามซ้ำ | เหลือเฉพาะการกดจริงบนหน้าจอ (U1) |
