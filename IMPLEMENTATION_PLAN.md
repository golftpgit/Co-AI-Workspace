...
**อัปเดต 2026-08-11 (รอบต่อทีม P4)**: U14 ปิดแล้ว · U15 เปิดอยู่และยังหาสาเหตุไม่ได้ · ที่ยังเปิดจริงๆ คือ **U4 U6 U7 U8 U12 U13 U15**
...
| **U15** | **ledger ที่ลง DB ไม่ตรงกับที่หน้าจอทีมแสดง** | 🔴 **ยังไม่ปิด** — รันจริงในแอป: หน้าจอแสดงถูกทุกสถานะ (รอบ 1 → 2 → 3 → escalate) แต่แถวใน `task` ค้างที่ `attempts: 1`, `findings: []` ทั้งที่ `updated_at` ขยับตอนท้ายรัน แปลว่า persist ถูกเรียกแต่เขียนค่าเก่าลงไป · **แก้ไปแล้วสองสาเหตุ**: escalate ไม่เคยถูกบันทึกเลย (รันจบตรงนั้นพอดี) และ specialist ที่ throw ก็ไม่เคยถูกบันทึก · `persist` เลิกใช้ `try?` แล้ว เพราะ ledger ที่หยุดอัปเดตเงียบๆ หน้าตาเหมือน ledger ที่ทันสมัย · **ยังเหลือ**: อาการยังเกิดในแอป แต่ไม่เกิดใน `EmbeddingCheck` ที่รัน orchestrator ตัวจริงกับ store ตัวจริง (ผ่าน 3 รอบถูกต้อง) — ยังหาสาเหตุไม่ได้ | หา root cause: unified log ไม่เก็บ log ของแอปเลย (`log show --predicate 'subsystem == "com.coaiworkspace.app"'` คืน 0 บรรทัด) ทำให้ error ที่ `persist` เพิ่งเริ่ม log ยังมองไม่เห็น — ต้องแก้เรื่อง log ก่อนถึงจะไล่ต่อได้ |
