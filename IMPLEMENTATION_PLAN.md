...
| **U16** | **แอปที่ sandbox มองไม่เห็นโมเดลที่เครื่องมีอยู่** *(เจอตอน P5.1, 2026-08-12)* | `LocalModelCatalog` เจอโมเดลสนทนาสองตัวใน `~/.lmstudio/models` ตอนรันจาก terminal แต่ในแอปจริง `homeDirectoryForCurrentUser` คือ container ในนั้นมีแต่ `bge-m3` (ซึ่งไม่มี chat template จึงถูกคัดออกถูกต้องแล้ว) — **`check.sh` ขึ้นว่ามีสองตัว แอปขึ้นว่าไม่มีเลย** และทั้งคู่พูดความจริง | P5.2: โหลดลงโฟลเดอร์ของแอปเอง (`AppPaths.modelsDirectory` อยู่ในลิสต์ค้นหาแล้ว) หรือให้ผู้ใช้เลือกโฟลเดอร์ผ่าน open panel แล้วเก็บ security-scoped bookmark |
