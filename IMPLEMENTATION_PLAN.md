...
| **P1.10** Chat UI (แกน) | หน้าคุย + streaming + conversation list + approval banner inline | คุยกับ agent จริงจนจบเทิร์นที่มี tool call + approval ได้ | ✅ `AgentTurnRunner` วิ่งครบเทิร์น (user→DB→history จาก DB→router→stream→tool→gate→approval→DB) + Chat UI ครบ sidebar/streaming/3 สวิตช์/approval banner ที่แก้ argument ก่อนอนุมัติได้ · **เทิร์นที่มี tool call + approval พิสูจน์ด้วย end-to-end test กับ DB/process/broker ของจริง** — ส่วน UI ยืนยันแค่ build+sign+launch (ยังไม่ได้ขับผ่านหน้าจอจริง) |
...
4. **ขับ Chat UI ด้วยมือหนึ่งรอบ** — logic ครบและมีเทสคุมแล้ว แต่ยังไม่มีใครกดจริงตั้งแต่เปิดแอปจนอนุมัติ tool
